...
class MaskCTLoss(object):
    ...
    def __call__(self, global_feat, mask_global_feat, labels, warmup_margin=False, print_data=False,
                 normalize_feature=False, mask=None):
        if normalize_feature:
            global_feat = normalize(global_feat, axis=-1)
            mask_global_feat = normalize(mask_global_feat, axis=-1)
        dist_mat = self.dist_func(global_feat, global_feat)
        dist_ap, dist_an = hard_example_mining(
            dist_mat, labels)

        mask_dist_mat = self.dist_func(mask_global_feat, mask_global_feat)
        mask_dist_ap, mask_dist_an = hard_example_mining(mask_dist_mat, labels)
        ...
        if mask is not None:
            ...
            mask_dist_ap = mask_dist_ap[mask]
            mask_dist_an = mask_dist_an[mask]

            y = y[mask]

        if self.margin is not None:
            loss = self.ranking_loss(dist_an + mask_dist_an, dist_ap + mask_dist_ap, y)
        else:
            loss = self.ranking_loss((dist_an + mask_dist_an) - (dist_ap + mask_dist_an), y)

        if print_data:
            print(f'LOSS: {loss.item()}')
            # precision
            prec = (dist_an + mask_dist_an > dist_ap + mask_dist_ap).data.float().mean()
            print(f'precision: {prec}')
            # the proportion of triplets that satisfy margin
            sm = (dist_an + mask_dist_an > dist_ap + mask_dist_ap + self.margin).data.float().mean()
            print(f'proportion of triplets that satisfy margin: {sm}')
            # average (anchor, positive) distance
            d_ap = (dist_ap + mask_dist_ap).data.mean()
            print(f'AP mean distance: {d_ap}')
            # average (anchor, negative) distance
            d_an = (dist_an + mask_dist_an).data.mean()
