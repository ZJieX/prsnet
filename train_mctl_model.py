...
from model.bases import ModelBase
from reid_utils.misc import run_main
...
class CTLModel(ModelBase):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=None):
        ...
        # Get backbone features, mask_feartures is new add
        _, features, mask_features = self.backbone(x)
        ...
        mask_features_real = mask_features[isReal]
        ...
        bn_mask_features = self.bn(mask_features_real)
        ...
        mask_masks = masks.to(mask_features.device)
        mask_masks = mask_masks & t_re

        masks_float = masks.float().to(features.device)
        padded = masks_float.unsqueeze(-1) * features.unsqueeze(0)  # For broadcasting

        mask_masks_float = mask_masks.float().to(mask_features.device)
        mask_padded = mask_masks_float.unsqueeze(-1) * mask_features.unsqueeze(0)
        ...
        mask_centroids_mask = rearrange(
            mask_masks, "i (ins s) -> i ins s", s=self.hparams.DATALOADER.NUM_INSTANCE
        )
        mask_padded_tmp = rearrange(
            mask_padded,
            "i (ins s) dim -> i ins s dim",
            s=self.hparams.DATALOADER.NUM_INSTANCE,
        )
        ...
        mask_valid_inst = mask_centroids_mask.sum(-1)
        mask_valid_inst_bool = mask_centroids_mask.sum(-1).bool()
        mask_centroids_emb = mask_padded_tmp.sum(-2) / mask_valid_inst.masked_fill(
            valid_inst == 0, 1
        ).unsqueeze(-1)
        ...
        for i in range(self.hparams.DATALOADER.NUM_INSTANCE):
            if valid_inst_bool[i].sum() <= 1 or mask_valid_inst_bool[i].sum() <= 1:
                continue
            ...
            mask_current_mask = mask_masks[i, :]
            mask_current_labels = class_labels[~mask_current_mask & t_re[i]]
            mask_query_feat = features[~mask_current_mask & t_re[i]]
            mask_current_centroids = mask_centroids_emb[i]
            mask_current_centroids = mask_current_centroids[
                torch.abs(mask_current_centroids).sum(1) > 1e-7
                ]

            embeddings_concat = torch.cat((query_feat, current_centroids))
            mask_embeddings_concat = torch.cat((mask_query_feat, mask_current_centroids))
            ...
            # )
            contrastive_loss, dist_ap, dist_an = self.mask_centroid_loss(
                embeddings_concat, mask_embeddings_concat, labels_concat
            )
