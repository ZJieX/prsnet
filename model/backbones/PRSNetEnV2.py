...
class MaskAutoencoderConvV2(nn.Module):
    def __init__(self, img_size=(256, 128), patch_size=(16, 8), in_chans=3, embed_dim=1024, in_planes=2048,
                 depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], drop_path_rate=0., layer_scale_init_value=1e-6):
        super(MaskAutoencoderConvV2, self).__init__()
        ...
        self.last_conv = nn.Conv2d(dims[-1], in_planes, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, mask_ratio):
        fearture = self.encoder(x, mask_ratio=mask_ratio)
        # pred = self.decoder(img_mask)
        # loss = self.forward_loss(x, pred, mask)
        print(fearture.shape)
