...
class MaskAutoencoderConv(nn.Module):
    def __init__(self, img_size=(256, 128), patch_size=(16, 8), in_chans=3, embed_dim=1024,
        ...
        for i in range(4):
            stage_decoder = nn.Sequential(
                *[Block(dim=dims[3 - i], drop_path=dp_rates[cur + j],
    ...
    def forward_econder(self, x, mask_ratio):
    ...
    def forward(self, x, mask_ratio):
        img_mask, mask = self.forward_econder(x, mask_ratio=mask_ratio)
        pred = self.forward_decoder(img_mask)
...
def RPSNet_tiny(weights, **kwargs):
    MAC = MaskAutoencoderConv(depths=[3, 3, 9, 3], dims=[96, 192, 384, 768])
    if weights != "":
        # net.load_state_dict(torch.load("/data/CNX_V3/model_data/convnext_base_1k_224_ema.pth")["net"])
        ...
        for k in list(weights_dict.keys()):
            if "head" in k:
                ...
                del weights_dict[k]
        MAC.load_state_dict(weights_dict, strict=False)
    return MAC


def RPSNet_small(weights, **kwargs):
    MAC = MaskAutoencoderConv(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768])
    if weights != "":
        # net.load_state_dict(torch.load("/data/CNX_V3/model_data/convnext_base_1k_224_ema.pth")["net"])
        print('Load weights from {}.'.format(weights))
        weights_dict = torch.load(weights)["model"]
