...
from reid_losses.center_loss import CenterLoss
from reid_losses.triplet_loss import CrossEntropyLabelSmooth, TripletLoss
from reid_losses.mask_centoid_loss import MaskCTLoss
from model.baseline import Baseline
from reid_solver import build_optimizer, build_scheduler
from reid_utils.reid_metric import R1_mAP
...
class ModelBase(pl.LightningModule):
    def __init__(self, cfg=None, test_dataloader=None, **kwargs):
        ...
        self.mask_centroid_loss = MaskCTLoss(
            self.hparams.SOLVER.MARGIN, self.hparams.SOLVER.DISTANCE_FUNC
        )
        ...
        self.center_loss = CenterLoss(
            num_classes=self.hparams.num_classes, feat_dim=d_model, use_gpu=self.use_cuda
        )
    ...
    def eval_on_train(self):
        if self.trainer.global_rank == 0 and self.trainer.local_rank == 0:
            ...
            for batch_idx, batch in enumerate(self.test_dataloader):
                x, class_labels, camid, idx = batch
                with torch.no_grad():
                    emb, _ = self.backbone(x.to(device))
                outputs.append(
